...
replaced by a different digit `v'`, and look at the model's next prediction.  A min-tracker follows the
corrupted value (it emits the smallest still-available digit ≥ `v'`); a counting sorter ignores the
prefix and emits the true `y_t`.  Only cases where the two answers differ are counted.
